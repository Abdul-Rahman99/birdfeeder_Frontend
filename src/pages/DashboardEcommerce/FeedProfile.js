...
import { BasicScatterChart } from "./ECharts";
import { Simple } from "./BubbleChart";
import { Datetime, ImagesChart } from "./ScatterCharts";
...
const FeedProfile = (props) => {
  ...
  const [RunningFeedSchedules, setRunningFeedSchedules] = useState();
  ...
  const updateVOLTCURRENT = () => {
    axios
      .get(api.API_URL + "/api/getVoltCurrentData/" + currentFeederId)
      .then((result) => {
  ...
  const getBirdsData = () => {
    axios
      .get(api.API_URL + "/api/getBirdsData/" + currentFeederId)
      .then((result) => {
  ...
  const getExportedData = () => {
    let url = api.API_URL + "/api/getExportedData/" + currentFeederId;
  ...
  const myTimer = (myTime) => {
    isTimerOn = true;
    ScheduleArray.push(myTime);
    console.log("Interval", "myTimers", myTime);
    ...
    var x = setInterval(function () {
      ...
      if (
        document.getElementById("days") != null &&
        parseInt(document.getElementById("days").innerHTML) === 0 &&
        document.getElementById("hours") != null &&
        parseInt(document.getElementById("hours").innerHTML) === 0 &&
        document.getElementById("minutes") != null &&
        parseInt(document.getElementById("minutes").innerHTML) === 0 &&
        document.getElementById("seconds") != null &&
        parseInt(document.getElementById("seconds").innerHTML) === 0
      ) {
        clearInterval(window.sessionStorage.getItem("intervalId"));
        console.log(
          "Interval cleared",
          window.sessionStorage.getItem("intervalId")
        );
        getSchedules();
        // getDeviceDetails()
      }
  ...
  const getFeedDateTimes = async () => {
    await axios
      .get(api.API_URL + "/api/getFeedDateTimes/" + currentFeederId)
      .then((result) => {
        setRunningFeedSchedules(result.data);
      })
      .catch((err) => console.log("Error:\n\n" + err));
  };
  const getDeviceDetails = async () => {
    await axios
      .get(api.API_URL + "/api/getDeviceDetails/" + currentFeederId)
      .then((result) => {
  ...
  const ReviseSchedules = (StrTimings) => {
    ...
    if (StrTimings && StrTimings != null) {
      ...
      for (let i = 0; i < StrTimings.length; i++) {
        console.log("Yup, next schedule is ", StrTimings[i]);
        console.log("Interval, See Schedule Array", ScheduleArray);
        var nextSchedule = moment(StrTimings[i]).format(
          "dddd, MMMM D, YYYY, H:mm:ss"
        );
        if (
          moment(nextSchedule).isAfter(CurrentdateString) &&
          ScheduleArray.includes(nextSchedule) === false
        ) {
          console.log("Yup, next schedule is ", nextSchedule);
          console.log("Yup, current time is ", CurrentdateString);
          console.log(
            "Interval",
            "next schedule is ",
            nextSchedule,
  ...
  const configureFeedTimings = (CmData, StrTimings) => {
    ...
    var nextTurn = moment(dateString)
      .add(CmData[68], "minutes")
      .format("dddd, MMMM D, YYYY, H:mm:ss");
    // if (isTimerOn) {
    // clearInterval(x)
    // }

    // Commented for next feed fix
    // clearInterval(window.sessionStorage.getItem("intervalId"))
    // resetFeedTimerHTML()
    // myTimer(nextTurn)
  ...
  const getFeedsTimings = async () => {
    await axios
      .get(api.API_URL + "/api/getFeedsTimings/" + currentFeederId)
      .then((result) => {
  ...
  const getSensorData = () => {
    setTankCapacity(0);
    axios
      .get(api.API_URL + "/api/getSensorData/" + currentFeederId)
      .then((result) => {
        setTankCapacity(result.quantity);
  ...
  const getDeviceCameras = () => {
    axios
      .get(api.API_URL + "/api/getDeviceCameras/" + currentFeederId)
      .then((result) => {
        setDeviceCameras(result.data);
      })
      .catch((err) => console.log("Error:\n\n" + err));
  };

  const getTankCapacity = () => {
    // setTankCapacity(feedQuantity)
  };
  const getTankCapacity_bk = (ping_dist) => {
    if (currentFeederId === 4 || parseInt(currentFeederId) === 4) {
      setTankCapacity(100);
    } else {
      if (ping_dist >= 64) {
        setTankCapacity(0);
  ...
  const mySocketSystem = () => {
    socket.on("connect", () => {
      ...
      socket.on("custom-event", (topic, message, feeder_id) => {
        console.log("received socket message: ", topic, feeder_id);
        ...
        if (
          ...
          console.log(currentFeederId, topic, message);
          if (parseInt(currentFeederId) === 3 || currentFeederId === "3") {
            // console.log(currentFeederId, topic, message.key);
            if (topic === "capture3") {
              // console.log("received socket message: ", topic)
              if (message.key != null && message.key !== "undefined") {
                // console.log("received socket message: ", message.key)
  ...
  const setFeedDay = (val, is_enabled) => {
    if (is_enabled === true) {
      if (feed_day2.includes(val) === false) feed_day2.push(val);
    } else {
  ...
  const getFullDesc = (feed_schedule, feed_time, feed_time_type, feed_day) => {
    ...
    if (feed_schedule !== "FixedTime") {
      ...
      return feed_day + " " + feed_time_type + " " + new_feed_time;
    } else {
      return feed_day + " at " + feed_time;
    }
  };
  ...
  const saveInfo = () => {
    ...
    axios
      .post(api.API_URL + "/api/addSchedule/" + currentFeederId, data)
      .then((res) => {
        toast("Feed Schedule Added Successfully", {
          position: "top-right",
          hideProgressBar: true,
          closeOnClick: false,
          className: "bg-success text-white",
  ...
  const getFeederStopStatus = () => {
    axios
      .get(api.API_URL + "/api/getFeederStopStatus/" + currentFeederId)
      .then((res) => {
        // console.log(res);
  ...
  const deleteSchedule = (id) => {
    ...
    if (window.confirm("Are you sure you wish to delete?")) {
      ...
      axios
        .post(
          api.API_URL + "/api/deleteSchedule/" + userId + "/" + currentFeederId,
          {
            id: id,
          }
        )
  ...
  const getSchedules = (id, value) => {
    // console.log("Schedule", id, (value ? "1" : "0"));
    axios
      .get(api.API_URL + "/api/getSchedules/" + currentFeederId)
      .then((res) => {
        // console.log(res);
  ...
  const ExecuteFeedNow = () => {
    ...
    axios
      .post(
        api.API_URL + "/api/ExecuteFeedNow/" + userId + "/" + currentFeederId
      )
      .then((res) => {
        // console.log(res);
    ...
    if (camPic == 0) {
      return (
        <div className="d-flex justify-content-center">
          <Spinner size="lg" color="light" className="me-2">
            {" "}
            Loading...{" "}
          </Spinner>
        </div>
  ...
  const updateFeedTime = () => {
    ...
    if (
      ...
      HoursMinutesList.includes(hourMin) === false
    ) {
  ...
  const validateField = () => {
    ...
    if (toProcess && toProcessQty && toProcessTimer && toProcessTitle) {
      ...
      if (!toProcessQty) {
        ...
        document.getElementById("errorbox").innerHTML =
          "Title must not have numbers or spaces";
    ...
    <React.Fragment>
              ...
              <Col xl={6}>
                <div className="form-check form-radio-primary mb-3">
                  <Input
                    ...
                    id="formradio_before"
                    value={"before"}
                    onChange={(e) => setFeedTimeType(e.target.value)}
                    defaultChecked={FeedTimeType === "before" ? true : false}
                  />
              ...
              <Col xl={6}>
                <div className="form-check form-radio-primary mb-3">
                  <Input
                    ...
                    value={"after"}
                    onChange={(e) => setFeedTimeType(e.target.value)}
                    defaultChecked={FeedTimeType === "after" ? true : false}
                  />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  type="checkbox"
                  id="formcheck_mon"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Mon")}
                />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  id="formcheck_tue"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Tue")}
                />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  id="formcheck_wed"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Wed")}
                />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  id="formcheck_thu"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Thu")}
                />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  id="formcheck_fri"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Fri")}
                />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  id="formcheck_sat"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Sat")}
                />
            ...
            <Col xl={3}>
              <div className="form-check form-check-success mb-3">
                <Input
                  ...
                  id="formcheck_sun"
                  onChange={(e) => setFeedDay(e.target.value, e.target.checked)}
                  defaultChecked={feed_day.includes("Sun")}
                />
      ...
      <div className="page-content">
        <Container fluid>
          ...
          {/* {JSON.stringify(DeviceCameras)} */}
          <Row>
            {/* {DeviceCameras.length> 0 && DeviceCameras.map((camera, index) => (
                            <Col lg={6} key={index}>
                      ...
                      {IsExpired ? (
                        ...
                        >
                          <div className="d-flex flex-column gap-2 align-items-stretch">
                            <span className="countdown-item" id="days">
                              00
                            </span>
                            <span className="countdown-label" id="days-label">
                              Days
                            </span>
                          </div>
                          <div className="d-flex flex-column gap-2 align-items-stretch">
                            <span className="countdown-item" id="hours">
                              00
                            </span>
                            <span className="countdown-label" id="hours-label">
                              Hours
                            </span>
                          </div>
                          <div className="d-flex flex-column gap-2 align-items-stretch">
                            <span className="countdown-item" id="minutes">
                              00
                            </span>
                            <span
                          ...
                          <div className="d-flex flex-column gap-2 align-items-stretch">
                            <span className="countdown-item" id="seconds">
                              00
                            </span>
                            <span
                              className="countdown-label"
                      ...
                      }
                      {
                        ...
                        showstopLoader ? (
                          ...
                          <Button
                            ...
                            className="btn-icon m-2"
                            style={{ float: "right" }}
                            onClick={() => UpdateScheduleStopStatus()}
                          >
                            {" "}
                ...
                <CardBody>
                  ...
                  <Row>
                    ...
                    <Col xl={6}>
                      ...
                      >
                        <strong>
                          Total Feed Consumed: {feedConsumptionData?.max * 2}{" "}
                          Kgs
                        </strong>
                      </div>
                    </Col>
          ...
          {/* <Row>
                        <Col lg={12}>
                            <Card>
                                ...
                                <CardBody>
                                    <Simple dataColors='["--vz-success", "--vz-danger"]' mydata={BirdsDataForGraph} />
                                </CardBody>
                            </Card>
                        </Col>
                    </Row> */}
                  ...
                  <Row>
                    ...
                    <Col lg={8} style={{ float: "right" }}>
                      <Row onLoad={setUpDefaultDateRange}>
                        ...
                        <Col xl={1}>
                          {myGraphLoader ? (
                            <Spinner size="lg" color="light"></Spinner>
                          ) : (
                            <Button
                              color="primary"
                              className="btn-icon"
                              onClick={() => getBirdsDataForGraph()}
                            >
                              {" "}
                              <i className="ri-filter-line" />
                            </Button>
          ...
          {/* <Row>
                        <Col xl={12}>
                            <Card>
                                ...
                                <CardBody>
                                    <SimpleDonut dataColors='["--vz-success", "--vz-warning", "--vz-danger"]' myData={BirdsPieDataForGraph} />
                                </CardBody>
                            </Card>
                        </Col>
                    </Row> */}
                        ...
                        <Col lg={12}>
                            <Card>
                                ...
                                <CardBody>
                                    ...
                                    <LinewithDataLabels dataColors='["--vz-primary", "--vz-success"]' type={'volt'} BirdsDataForGraph={BirdsDataForGraph} />
                                </CardBody>
                            </Card>
                        </Col>
                    </Row> */}
