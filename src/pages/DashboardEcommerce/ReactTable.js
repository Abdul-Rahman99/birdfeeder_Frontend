...
import { Badge, Spinner, Alert, Button, Card, CardBody, Col, Container, Input, Modal, ModalBody, ModalHeader, PopoverBody, PopoverHeader, Row, UncontrolledPopover, UncontrolledTooltip } from 'reactstrap';
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: (cellProps) => {
          return (
            <span className="fw-semibold">{cellProps.endangered === 0 ? "-" : "Endangered"}</span>
          )
        },
        disableFilters: true,
        filterable: false,
      },
      {
        ...
        accessor: (cellProps) => {
          return (
            <span className="fw-semibold">{moment(cellProps.createdAt).format('YYYY-MM-DD h:mm:ss a')}</span>
          )
        },
        disableFilters: true,
        filterable: false,
    ...
    () => [
      {
        ...
        accessor: (cellProps) => {
          return (
            <Link to="/feed-profile" state={{ feederId: cellProps.id }} className="fw-medium" > {cellProps.title}</Link>
          )
        },
        disableFilters: true,
        filterable: true,
