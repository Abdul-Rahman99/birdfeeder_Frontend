...
import { Dropdown, DropdownMenu, TabContent, TabPane, Badge, Card, CardBody, CardHeader, Col, Container, Row, Label, Input, Modal, ModalBody, ModalHeader, ModalFooter, Table, Button } from 'reactstrap';
...
const DashboardEcommerce = (props) => {
    ...
    const getAllNotificationsList = () => {
        axios.get(api.API_URL + "/api/getAllNotificationsList")
            .then(res => {
    ...
    const getFeedLevelData = () => {
        axios.get(api.API_URL + "/api/getFeedLevelData")
            .then(res => {
                // console.log(res);
    ...
    const getSchedulesSummary = () => {
        axios.get(api.API_URL + "/api/getSchedulesSummary")
            .then(res => {
                // console.log(res);
    ...
    return (
        <React.Fragment>
            ...
            <Modal
                ...
                centered
            >
                <ModalHeader className="modal-title" id="staticNotificationLabel" toggle={() => {
                    tog_notification();
                }}>
                    Notification Detail
                ...
                <ModalBody className="p-5">
                    ...
                    <Row>
                        ...
                        <Col xl={9}>
                            {modalNotificationData?.message_text}
                        </Col>
                    </Row>
                    <Row>
                        <Col xl={3}>
                        ...
                        <Col xl={9}>
                            {getTime(modalNotificationData?.createdAt)}
                        </Col>
                    </Row>
                ...
                <ModalFooter>
                    ...
                    <div className="hstack gap-2 justify-content-center">
                        <button className="btn btn-primary" onClick={() => tog_notification()}>Close</button>
                    </div>
                </ModalFooter>
            </Modal>
            {/* Modal code exited */}
            ...
            >
                ...
                <ModalFooter>
                    ...
                    <div className="hstack gap-2 justify-content-center">
                        <button className="btn btn-primary" onClick={() => tog_backdrop()}>Close</button>
                    </div>
                </ModalFooter>
            </Modal>
            {/* Modal code exited */}
            ...
            <div className="page-content">
                <Container fluid>
                    <Row>
                        <Col>
                            <div className="h-100">
                                <Section rightClickBtn={toggleRightColumn} notificationsData={notificationsData} />
                                <Row>
                                    <Col xl={6} md={6} >
                                            ...
                                            <CardBody>
                                                <div className="d-flex align-items-center">
                                                    <div className="flex-grow-1 overflow-hidden">
                                                        <p className="text-uppercase mb-0">Total Active Devices</p>
                                                    </div>
                                                </div>
                                                    ...
                                                    >
                                                        {
                                                            feedLevelData.all?.map((val, i) => {
                                                                return <Marker key={i} title={val.title} position={setupCoordinates(val.other_info)}
                                                                    onClick={(props, marker) => {

                                                                        // to = "/feed-profile" state = {{ feederId: selectedElement.feeder_id }

                                                                        navigate("/feed-profile", { state: { feederId: val.id } })
                                            ...
                                            <CardBody style={{ padding: "0" }}>
                                                {
                                                    Object.keys(notificationsData).length > 0 ?
                                                        (
                                                            <SimpleBar style={{ maxHeight: "330px" }} >

                                                                {
                                                                    notificationsData.map((val, i) => {
                                                                        return <div key={i} className="text-reset notification-item d-block dropdown-item position-relative" style={{ cursor: "pointer" }} >
                                                                            <div className="d-flex">
                                                                                <div className="flex-grow-1">
                                                                                    <h6 className="mt-0 mb-2 lh-base stretched-link" onClick={() => {
                                                                                        setModalNotificationData(val);
                                                                                        tog_notification()
                                                                                    }}><strong>{val.title}</strong>&nbsp;{val.message_text}</h6>
                                                                                    <p className="mb-0 fs-11 fw-medium text-uppercase text-muted">
                                                                                        <span><i className="mdi mdi-clock-outline"></i> {getTime(val.createdAt)} </span>
                                                                                    </p>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                            ...
                                                            <TabPane tabId="1" className="p-4" style={{ display: "block" }}>
